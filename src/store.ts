...
export function generateId(): string { return `order_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`; }
...
export async function getRobots(): Promise<Robot[]> {
  ...
  return (data || []).map((p: any) => ({ ...p, pairs: typeof p.pairs === 'string' ? JSON.parse(p.pairs) : p.pairs || [], specs: typeof p.specs === 'string' ? JSON.parse(p.specs) : p.specs || [] }));
}
export async function getRobot(id: string): Promise<Robot | undefined> {
  const { data, error } = await supabase.from('products').select('*').eq('id', id).single();
  if (error || !data) return undefined;
  return { ...data, pairs: typeof data.pairs === 'string' ? JSON.parse(data.pairs) : data.pairs || [], specs: typeof data.specs === 'string' ? JSON.parse(data.specs) : data.specs || [] };
}
export async function saveRobot(robot: Partial<Robot> & { id: string }): Promise<void> {
  const row = { ...robot, pairs: JSON.stringify(robot.pairs || []), specs: JSON.stringify(robot.specs || []) };
...
export async function getEAFile(robotId: string): Promise<{ hasFile: boolean; fileName?: string; size?: number }> {
  const { data } = await supabase.storage.from(STORAGE_BUCKET).list(undefined, { search: robotId });
